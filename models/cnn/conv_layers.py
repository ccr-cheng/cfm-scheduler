...
def get_act(nonlinearity):
    if nonlinearity.lower() == 'elu':
        ...
        raise NotImplementedError(f'activation function {nonlinearity} does not exist!')
...
def stride_conv3x3(in_planes, out_planes, kernel_size=3, bias=True, spec_norm=False):
...
class UpsampleConv(nn.Module):
    def __init__(self, input_dim, output_dim, kernel_size=3, biases=True, spec_norm=False):
        super().__init__()
        self.conv = nn.ConvTranspose2d(input_dim, output_dim, kernel_size, stride=2, padding=kernel_size // 2,
                                       output_padding=1, bias=biases)
        if spec_norm:
            self.conv = spectral_norm(self.conv)
...
class ResidualBlock(nn.Module):
    def __init__(self, input_dim, output_dim, resample=None, act=nn.ELU(),
        ...
        if resample == 'down':
            ...
            conv_shortcut = partial(stride_conv3x3, spec_norm=spec_norm)
            self.conv1 = stride_conv3x3(input_dim, output_dim, spec_norm=spec_norm)
            self.normalize2 = normalization(output_dim)
            self.conv2 = conv3x3(output_dim, output_dim, spec_norm=spec_norm)

        elif resample == 'up':
            conv_shortcut = partial(UpsampleConv, spec_norm=spec_norm)
            self.conv1 = UpsampleConv(input_dim, output_dim, spec_norm=spec_norm)
            self.normalize2 = normalization(output_dim)
            self.conv2 = conv3x3(output_dim, output_dim, spec_norm=spec_norm)
        else:
            raise Exception('invalid resample value')
